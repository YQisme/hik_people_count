...
import { buildApiUrl, buildChannelApiUrl } from '../utils/api'
...
const channelId = computed(() => `${route.params.channelId ?? ''}`.trim())
const selectedChannelName = computed(() => {
  const current = channels.value.find((item) => item.id === channelId.value)
  return current?.name || channelId.value || '通道看板'
})
...
async function loadHistoryDevices() {
  const response = await fetch(buildChannelApiUrl('/api/devices', channelId.value), {
...
async function loadDashboard() {
  ...
  try {
    const response = await fetch(buildChannelApiUrl('/api/dashboard', channelId.value), {
      headers: {
        Accept: 'application/json',
      },
    })
...
function connectDashboardStream() {
  ...
  const source = new EventSource(buildChannelApiUrl('/api/dashboard/stream', channelId.value))
...
onMounted(() => {
  ...
  void loadChannels()
  void initializeChannelDashboard()
  loadSavedLimit()
  void loadStayWarningMinutes()
})

async function initializeChannelDashboard() {
  if (!channelId.value) {
    router.replace('/')
    return
  }

  historyDeviceIP.value = ''
  loadError.value = ''
  disconnectDashboardStream()
  await loadDashboard()
  ...
  <main class="screen" :class="{ 'screen--area-alert-active': areaAlertActive }">
    ...
    <header class="topbar panel">
      <div>
        ...
        <p class="topbar__subtitle">通道详情看板</p>
      </div>
      <div class="topbar__meta">
        <button class="back-button" type="button" @click="backToOverview">返回总览</button>
...
.back-button {
  border: 1px solid rgba(88, 196, 255, 0.45);
...
.back-button:hover {
  background: rgba(16, 58, 92, 0.92);
  border-color: rgba(110, 216, 255, 0.8);
}
