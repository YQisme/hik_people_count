...
{
    public class AreaAlertMonitorService
    {
        private readonly RuntimeConfig _config;
        private Thread _thread;
        private bool _running;

        public AreaAlertMonitorService(RuntimeConfig config)
        {
        ...
        private void RunLoop()
        {
            while (_running)
            {
                if (!_config.AreaAlertMqttEnabled || string.IsNullOrEmpty(_config.AreaAlertMqttHost) || string.IsNullOrEmpty(_config.AreaAlertMqttTopic))
                {
                    Thread.Sleep(5000);
                    continue;
                }
                ...
                try
                {
                    ...
                    IAsyncResult connectResult = client.BeginConnect(_config.AreaAlertMqttHost, _config.AreaAlertMqttPort, null, null);
                    if (!connectResult.AsyncWaitHandle.WaitOne(4000, false))
                    {
                        throw new IOException("连接区域报警MQTT超时");
                    }
                    ...
                    ReadConnAck(stream);
                    WritePacket(stream, BuildSubscribePacket(_config.AreaAlertMqttTopic, 1));
                    ReadSubAck(stream);
                    Console.WriteLine("区域报警MQTT已订阅: " + _config.AreaAlertMqttHost + ":" + _config.AreaAlertMqttPort + " / " + _config.AreaAlertMqttTopic);
                ...
                catch (Exception ex)
                {
                    if (_running)
                    {
                        Console.WriteLine("区域报警MQTT监听异常: " + ex.Message);
                        AreaAlertState.MarkDisconnected(_config.AreaAlertMqttTopic);
                        Thread.Sleep(3000);
                    }
                }
                finally
        ...
        private void HandlePublish(byte header, byte[] body)
        {
            ...
            int hasPeople = ExtractIntValue(payload, "hasPeople", 1);
            string zoneName = ExtractStringValue(payload, "zoneName");
            bool isActive = true;

            AreaAlertState.Update(isActive, hasPeople, zoneName, topic, payload);
            Console.WriteLine("收到区域报警MQTT: zone=" + zoneName + ", hasPeople=" + hasPeople + ", active=" + (isActive ? "1" : "0"));
        }
        ...
        private byte[] BuildConnectPacket()
        {
            using (var body = new MemoryStream())
            {
                ...
                WriteMqttString(body, string.IsNullOrEmpty(_config.AreaAlertMqttClientId) ? "acs-area-alert-subscriber" : _config.AreaAlertMqttClientId);
                if (!string.IsNullOrEmpty(_config.MqttUsername))
                {
